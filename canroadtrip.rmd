...
```{r}
# Render map with red route, blue stop dots, and yellow highlight for active stop
output$map <- renderMaplibre({
  m <- maplibre()

  # Route: connect all stops in order
  if (nrow(stops_sf) >= 2) {
    ...
    m <- m |>
      ...
      add_line_layer("route_line", "route", line_color = "#D22", line_width = 4) # red
  }

  # Stops: all blue dots + highlight layer for current stop
  m |>
    ...
    add_circle_layer("stop_highlight", "stops",
      circle_radius = 10, circle_color = "#FFCC00", # yellow
      circle_stroke_color = "#333333", circle_stroke_width = 2
    ) |>
    fit_bounds(stops_sf, padding = 50)
})

# Scroll handlers: fly to current stop + highlight it
